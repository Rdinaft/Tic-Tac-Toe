...
def first_turn_move(
    ...
    for line in range(num_of_lines):
        if field[line] == [None] * num_of_lines and chosen_side == Sign.crosses.name:
            return take_coordinates_from_player(player_sign, field)


def take_coordinates_from_player(
    player_sign: Sign, field: list[list[Sign | None]], sign_not_placed: bool = True
) -> None:
    print("---------Your turn!---------")
...
def check_for_sign_in_coordinate(
    field: list[list[Sign | None]], line_coordinate: int, column_coordinate: int
) -> bool:
